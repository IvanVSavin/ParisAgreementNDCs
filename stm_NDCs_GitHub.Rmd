...
```{r echo=FALSE}
rm(list=ls())
setwd("E:/Projects/3_CurrentProjects/NDC text analysis/ndc-master")
...
setwd("E:/Projects/3_CurrentProjects/NDC text analysis/NDCfinaladdition")
...
data_extra<-read.csv('E:/Projects/3_CurrentProjects/NDC text analysis/NDC data for Ivan extended.csv', header=T, sep=',')
...
load("E:/Projects/3_CurrentProjects/NDC text analysis/calibration_twitter_50v2311.RData")
...
if(justloaddata==0){
  ...
  save(ncpSelect2, file = "ndc_v2311_21test.RData")
}
...
tiopiclabel[18]<-"T1: Sustainable development" #ABM, numerical stochastic, bayesian
tiopiclabel[12]<-"T2: Economic development"
tiopiclabel[17]<-"T3: Greenhouse gas accounting" # in value creation
tiopiclabel[7]<-"T4: International support and funding" # in value creation
tiopiclabel[8]<-"T5: Emission scenarios" #ABM, numerical stochastic, bayesian
tiopiclabel[14]<-"T6: Adaptation planning" #progress, patents
tiopiclabel[16]<-"T7: Vulnerability of coastal areas"
tiopiclabel[9]<-"T8: Targets and reporting" #analysing 
tiopiclabel[20]<-"T9: Climate system impacts" #developing 
tiopiclabel[4]<-"T10: Green energy policies"# and roadmapping
tiopiclabel[15]<-"T11: Goals and commitments" #sme, entrepreneurship, csr
tiopiclabel[3]<-"T12: Green energy technologies" #decision making
tiopiclabel[19]<-"T13: Forests and ecosystems" #analysing 
tiopiclabel[5]<-"T14: Project financing" # public policy, science, technology and innovation
tiopiclabel[21]<-"T15: Rural development"
tiopiclabel[6]<-"T16: Agriculture resource management"
tiopiclabel[13]<-"T17: Waste, transport and industry" #supply chain, agile, digitalization
tiopiclabel[11]<-"T18: (Inter)governmental organisations"
tiopiclabel[2]<-"T19: Gender and youth" #media, interaction, behaviour, online shopping
tiopiclabel[10]<-"T20: Action plans" #developing 
tiopiclabel[1]<-"T21: Agriculture mitigation" #"Undesirable versus desirable societies" "CRITICAL LONG-TERM PLANNING; DESIRABLE SOCIETIES; TECHNOLOGICAL FORECASTING"  
# tiopiclabel[16]<-"T16: golf countries (mining, women,...)" # literature related discovery, mining, literature review, AI
# tiopiclabel[22]<-"T22: "
...
#                             out2$vocab, 
#                             K = Kchoice,
#                             prevalence =~ version + Conditionality + Epc + Egdp + Echange + AFRC + ASIA + BASC + CISP + EURP  + LACA + OECD + SIDS,
#                             data = out2$meta,
#                             runs=2,
#                             seed =1,
#                             init.type="Spectral",
...
# 
# 
#   save(processed2,out2,ncpPrevFit_startup2, file = "secondstm.RData")
# }
# load("secondstm.RData")


#unique(data_lemmatized_split$id)
...
g11<-ggplot(data = data_clustering2, aes(x = Cluster, y = T11))+ 
  ...
  #               position= position_dodge(0.5), width = 0.2) +
  geom_point(position = position_dodge(0.5)) +
  labs(x = "", y = "T11: Goals and commitments")+
  theme(axis.text.x = element_text(size = 6, angle = 55, hjust = 1))+   theme(axis.title.y = element_text(size = 6))+
...
data_additional<- read.csv('E:/Projects/3_CurrentProjects/NDC text analysis/NDC data for Ivan etxtended March 2024.csv', header=T, sep=',',encoding = "UTF-8")
